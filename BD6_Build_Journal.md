...
## 2026-08-31 — Session 36-CC (Claude Code, parallel track) — the ESCI slice: D-103's deferral closes on evidence
**TC-first (5 tests, RED before the slice existed):** shape, standard-loader compatibility, graded label mapping bounds, judged-docs-in-corpus closure, provenance manifest with both source sha256s.
**D-104 (the pinning decision D-103 said was needed):** us · small_version=1 · test split, FIRST 200 query_ids ascending — a rule, not a sample — corpus = exactly the judged products, E→3/S→2/C→1/I→0. Derived from the full 1.1GB parquets (downloaded once, sha256-pinned in the MANIFEST) by a committed maintainer script whose pyarrow/pandas deps stay deliberately out of pyproject. The slice itself is COMMITTED (6.6MB, Apache-2.0 attributed): users never download ESCI, and make bench gains the dataset with zero new fetch steps.
**The run:** all three datasets in one command (~12 min). scifact and nfcorpus reproduced BIT-IDENTICALLY against the previous run — cross-run determinism now witnessed, and the launch-draft guardrail numbers stand untouched. ESCI: dense 0.415 nDCG@10 / 0.812 recall@100; hybrid loses -0.191 / -0.111 — the third dataset, the same honest pattern, published at full prominence. The generic-schema story is now consistent across scientific abstracts, medical queries and product search.
**Guardrails after regeneration:** launch-kit number-traceability 5/5 against the new RESULTS.md; whole suite 326 green.
**With this, TC-05 is met in full on evidence — the D-103 deferral is closed, not ruled around.** The entire remaining open surface of the project: TC-10 (Rainbird, proprietary) and TC-12/T-052 (the maintainer publishing).
